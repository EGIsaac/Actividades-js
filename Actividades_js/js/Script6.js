// Variable global para almacenar el tiempo actual
let time = new Date();
let deltaTime = 0;
...
function DecidirCrearObstaculos() {
    ...
    if (tiempoHastaObstaculo <= 0) {
        CrearObstaculo(); // Llama a la función para crear un nuevo obstáculo
    }
}
...
function DecidirCrearNubes() {
    ...
    if (tiempoHastaNube <= 0) {
        CrearNube(); // Llama a la función para crear una nueva nube
    }
}
...
function MoverObstaculos() {
    for (let i = obstaculos.length - 1; i >= 0; i--) {
        if (obstaculos[i].posX < -obstaculos[i].clientWidth) { // Si el obstáculo ha salido del contenedor
...
function MoverNubes() {
    for (let i = nubes.length - 1; i >= 0; i--) {
        if (nubes[i].posX < -nubes[i].clientWidth) { // Si la nube ha salido del contenedor
